...
from engine.rules.base import REGISTRY, Finding, Rule
from engine.run import discover_rules, run_all
from tests.rules.conftest import make_query, make_snapshot
...
def test_dummy_rule_needs_only_registration(monkeypatch):
    ...
    monkeypatch.setattr("engine.run.REGISTRY", [*REGISTRY, AlwaysFires])
...
def test_findings_sorted_by_severity_then_impact():
    ...
    assert q1[0].evidence["pct_of_db_time"] == 75.0
