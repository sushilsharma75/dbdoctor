...
import engine.rules as _rules_pkg
from engine.models import EngineName, Snapshot
from engine.rules.base import REGISTRY, Finding, Rule
from engine.score import HealthScore, compute_score, sort_findings
...
def discover_rules() -> list[type[Rule]]:
    ...
    if not _discovered:
        for mod in pkgutil.iter_modules(_rules_pkg.__path__):
            if mod.name != "base":
                importlib.import_module(f"engine.rules.{mod.name}")
        _discovered = True
