...
from engine.rules.base import Finding, Rule, fmt_bytes, register
...
class ConnectionHeadroom(Rule):
    ...
    def evaluate(self, snapshot: Snapshot) -> list[Finding]:
        ...
        if conn is None or conn.max_limit <= 0:
            return []
        observed = conn.peak if conn.peak is not None else conn.current
        basis = "peak_connections" if conn.peak is not None else "current_connections"
    ...
    def evaluate(self, snapshot: Snapshot) -> list[Finding]:
        ...
        for t in snapshot.tables:
            ...
            if t.size_bytes < T.g1_min_size_bytes:
                continue
            projected_90d = t.size_bytes * (1 + t.growth_30d_pct / 100) ** 3
            findings.append(
                self.finding(
                    snapshot,
                    severity="MEDIUM",
                    ...
                    suggested_action=(
                        "At the observed growth rate this table triples its share of "
                        "storage within a quarter. Decide now whether old rows can be "
