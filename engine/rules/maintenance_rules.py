...
class AutovacuumNotRunning(Rule):
    id = "R-M2"
    title = "Autovacuum is disabled or has never processed an active table"
    category = "maintenance"
    engines = PG_ONLY
        ...
        for t in snapshot.tables:
            ...
            findings.append(
                self.finding(
                    ...
                    suggested_action=(
                        "This table accumulates dead rows but autovacuum has never "
                        "processed it — its thresholds are likely too high for a table "
